        ...
        for (int i = 0; i < inputList.size(); i++) // swaps every index val for each element in the array
         {
            ...
            //swapping
			String temp = inputList.get(randomIndexToSwap); // holds the value from the random index  in a temporary variable
			inputList.set(randomIndexToSwap,inputList.get(i)); // sets the task at the current index to the random index position
			inputList.set(i, temp); // current value index is reassigned to the random value
		}


        int endIndex = Math.min(amountTask, inputList.size()); // incase user wants the list length to be returned is greater than the list, we get the min val
